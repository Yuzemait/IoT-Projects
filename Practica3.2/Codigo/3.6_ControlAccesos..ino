// Práctica hecha por Guadalupe Torres - clase IoT || Editado por Jose Cota
...
byte Usuario1[4] = {0xA6, 0x62, 0x6A, 0x33}; // UID tarjeta CON acceso (reemplaza por tu UID)
byte Usuario2[4] = {0xB3, 0x39, 0x9C, 0xE2}; // UID de otra tarjeta que quieras que tenga acceso
  ...
  if (mfrc522.PICC_IsNewCardPresent()) {
    ...
    if (mfrc522.PICC_ReadCardSerial()) {
      ...
      if (compareArray(ActualUID, Usuario1)) {
        ...
        Serial.println("Acceso concedido...");
        Serial.println("Bienvenido tarjeta blanca");
        digitalWrite(greenLed, HIGH); // Enciende el LED verde
        delay(1500);                   // Espera 1.5 segundos
        digitalWrite(greenLed, LOW);  // Apaga el LED verde
      } else {
